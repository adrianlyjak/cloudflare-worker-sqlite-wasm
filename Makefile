...
prerelease: install all
	cp -r package.json package-lock.json dist README.md LICENSE build/
...
.PHONY: optimized
optimized: dist/sql-asm.js dist/sql-wasm.js dist/sql-cf-wasm.js dist/sql-asm-memory-growth.js
...
dist/sql-cf-wasm.js: $(BITCODE_FILES) $(OUTPUT_WRAPPER_FILES) $(SOURCE_API_FILES) $(EXPORTED_METHODS_JSON_FILES)
	$(EMCC) $(EMFLAGS) $(EMFLAGS_OPTIMIZED) $(EMFLAGS_WASM) $(EMFLAGS_CF_WORKER) $(BITCODE_FILES) $(EMFLAGS_PRE_JS_FILES) -o $@
	mv $@ out/tmp-raw.js
	cat src/shell-pre.js out/tmp-raw.js src/shell-post.js > $@
	rm out/tmp-raw.js
